...
/**
 * This file contains the customcertpwt element digitial signature's core interaction API.
 *
 * @package    customcertpwtelement_digitalsignature
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_digitalsignature;

defined('MOODLE_INTERNAL') || die();

/**
 * The customcertpwt element digital signature's core interaction API.
 *
 * @package    customcertpwtelement_digitalsignature
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
 */
class element extends \customcertpwtelement_image\element {
    ...
    public function render_form_elements($mform) {
        ...
        $mform->addElement('select', 'signaturefileid', get_string('digitalsignature', 'customcertpwtelement_digitalsignature'),
            self::get_signatures());

        $mform->addElement('text', 'signaturename', get_string('signaturename', 'customcertpwtelement_digitalsignature'));
        $mform->setType('signaturename', PARAM_TEXT);
        $mform->setDefault('signaturename', '');

        $mform->addElement('passwordunmask', 'signaturepassword',
            get_string('signaturepassword', 'customcertpwtelement_digitalsignature'));
        $mform->setType('signaturepassword', PARAM_TEXT);
        $mform->setDefault('signaturepassword', '');

        $mform->addElement('text', 'signaturelocation', get_string('signaturelocation', 'customcertpwtelement_digitalsignature'));
        $mform->setType('signaturelocation', PARAM_TEXT);
        $mform->setDefault('signaturelocation', '');

        $mform->addElement('text', 'signaturereason', get_string('signaturereason', 'customcertpwtelement_digitalsignature'));
        $mform->setType('signaturereason', PARAM_TEXT);
        $mform->setDefault('signaturereason', '');

        $mform->addElement('text', 'signaturecontactinfo',
            get_string('signaturecontactinfo', 'customcertpwtelement_digitalsignature'));
        ...
        $mform->addElement('filemanager', 'digitalsignature',
            get_string('uploaddigitalsignature', 'customcertpwtelement_digitalsignature'), '',
            $this->signaturefilemanageroptions);
    }
    ...
    public function save_form_elements($data) {
        ...
        \mod_customcertpwt\certificate::upload_files($data->digitalsignature, $context->id, 'signature');
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'mod_customcertpwt', 'signature', 0,
            $this->signaturefilemanageroptions);
    ...
    public static function get_signatures() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'mod_customcertpwt', 'signature', false,
                'filename', false)) {
        ...
        if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'mod_customcertpwt', 'signature', false,
                'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = $file->get_filename();
            }
        ...
        $arrfiles = array('0' => get_string('nosignature', 'customcertpwtelement_digitalsignature')) + $arrfiles;
    ...
    public function get_signature_file() {
        ...
        return $fs->get_file($imageinfo->signaturecontextid, 'mod_customcertpwt', $imageinfo->signaturefilearea,
