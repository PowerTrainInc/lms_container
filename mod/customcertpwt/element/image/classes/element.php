...
/**
 * This file contains the customcertpwt element image's core interaction API.
 *
 * @package    customcertpwtelement_image
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_image;

defined('MOODLE_INTERNAL') || die();

/**
 * The customcertpwt element image's core interaction API.
 *
 * @package    customcertpwtelement_image
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class element extends \mod_customcertpwt\element {
    ...
    public function render_form_elements($mform) {
        $mform->addElement('select', 'fileid', get_string('image', 'customcertpwtelement_image'), self::get_images());

        $mform->addElement('text', 'width', get_string('width', 'customcertpwtelement_image'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'width', 'customcertpwtelement_image');

        $mform->addElement('text', 'height', get_string('height', 'customcertpwtelement_image'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'height', 'customcertpwtelement_image');
        ...
        $mform->addElement('select', 'alphachannel', get_string('alphachannel', 'customcertpwtelement_image'), $alphachannelvalues);
        $mform->setType('alphachannel', PARAM_FLOAT);
        $mform->setDefault('alphachannel', 1);
        $mform->addHelpButton('alphachannel', 'alphachannel', 'customcertpwtelement_image');

        if (get_config('customcertpwt', 'showposxy')) {
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] < 0)) {
            $errors['width'] = get_string('invalidwidth', 'customcertpwtelement_image');
        }

        // Check if height is not set, or not numeric or less than 0.
        if ((!isset($data['height'])) || (!is_numeric($data['height'])) || ($data['height'] < 0)) {
            $errors['height'] = get_string('invalidheight', 'customcertpwtelement_image');
        }
    ...
    public function render_html() {
        ...
        // Get the image.
        $fs = get_file_storage();
        if ($file = $fs->get_file($imageinfo->contextid, 'mod_customcertpwt', $imageinfo->filearea, $imageinfo->itemid,
                $imageinfo->filepath, $imageinfo->filename)) {
            $url = \moodle_url::make_pluginfile_url($file->get_contextid(), 'mod_customcertpwt', 'image', $file->get_itemid(),
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'mod_customcertpwt', 'image', 0, $this->filemanageroptions);
    ...
    public function after_restore($restore) {
        ...
        // Perform the update.
        $DB->set_field('customcertpwt_elements', 'data', $elementinfo, array('id' => $this->get_id()));
    }
    ...
    public function get_file() {
        ...
        return $fs->get_file($imageinfo->contextid, 'mod_customcertpwt', $imageinfo->filearea, $imageinfo->itemid,
            $imageinfo->filepath, $imageinfo->filename);
    }
    ...
    public static function get_images() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'mod_customcertpwt', 'image', false, 'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = get_string('systemimage', 'customcertpwtelement_image', $file->get_filename());
            }
        }
        // Loop through the files uploaded in the course context.
        if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'mod_customcertpwt', 'image', false,
            'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = get_string('courseimage', 'customcertpwtelement_image', $file->get_filename());
            }
        }

        \core_collator::asort($arrfiles);
        $arrfiles = array('0' => get_string('noimage', 'customcertpwt')) + $arrfiles;
    ...
    public function copy_element($data) {
        ...
        if (isset($imagedata->filearea)) {
            ...
            $DB->set_field('customcertpwt_elements', 'data', $this->save_unique_data($imagedata), ['id' => $this->get_id()]);
        }
