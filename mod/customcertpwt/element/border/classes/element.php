...
/**
 * This file contains the customcertpwt element border's core interaction API.
 *
 * @package    customcertpwtelement_border
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_border;

defined('MOODLE_INTERNAL') || die();

/**
 * The customcertpwt element border's core interaction API.
 *
 * @package    customcertpwtelement_border
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class element extends \mod_customcertpwt\element {
    ...
    public function render_form_elements($mform) {
        ...
        $mform->addElement('text', 'width', get_string('width', 'customcertpwtelement_border'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->addHelpButton('width', 'width', 'customcertpwtelement_border');

        // The only other thing to define is the colour we want the border to be.
        \mod_customcertpwt\element_helper::render_form_element_colour($mform);
    }
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] <= 0)) {
            $errors['width'] = get_string('invalidwidth', 'customcertpwtelement_border');
        }

        // Validate the colour.
        $errors += \mod_customcertpwt\element_helper::validate_form_element_colour($data);
