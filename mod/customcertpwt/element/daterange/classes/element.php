...
/**
 * This file contains the customcertpwt date range element.
 *
 * @package    customcertpwtelement_daterange
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_daterange;

use \mod_customcertpwt\element_helper;
...
/**
 * The customcertpwt date range element.
 *
 * @package    customcertpwtelement_daterange
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class element extends \mod_customcertpwt\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[self::DATE_ISSUE] = get_string('issueddate', 'customcertpwtelement_daterange');
        $dateoptions[self::DATE_CURRENT_DATE] = get_string('currentdate', 'customcertpwtelement_daterange');
        $dateoptions[self::DATE_COMPLETION] = get_string('completiondate', 'customcertpwtelement_daterange');
        $dateoptions[self::DATE_COURSE_START] = get_string('coursestartdate', 'customcertpwtelement_daterange');
        $dateoptions[self::DATE_COURSE_END] = get_string('courseenddate', 'customcertpwtelement_daterange');
        $dateoptions[self::DATE_COURSE_GRADE] = get_string('coursegradedate', 'customcertpwtelement_daterange');

        $dateoptions = $dateoptions + element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'customcertpwtelement_daterange'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'customcertpwtelement_daterange');

        parent::render_form_elements($mform);

        $mform->addElement('header', 'dateranges', get_string('dateranges', 'customcertpwtelement_daterange'));
        $mform->addElement('static', 'help', '', get_string('help', 'customcertpwtelement_daterange'));
        $mform->addElement('static', 'placeholders', '', get_string('placeholders', 'customcertpwtelement_daterange'));

        $mform->addElement('text', 'fallbackstring', get_string('fallbackstring', 'customcertpwtelement_daterange'));
        $mform->addHelpButton('fallbackstring', 'fallbackstring', 'customcertpwtelement_daterange');
        ...
        $ranges[] = $mform->createElement(
            'date_selector',
            'startdate',
            get_string('start', 'customcertpwtelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'date_selector',
            'enddate',
            get_string('end', 'customcertpwtelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'checkbox',
            'recurring',
            get_string('recurring', 'customcertpwtelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'text',
            'datestring',
            get_string('datestring', 'customcertpwtelement_daterange'),
            ['class' => 'datestring']
        );
        ...
        $ranges[] = $mform->createElement(
            'advcheckbox',
            'rangedelete',
            get_string('setdeleted', 'customcertpwtelement_daterange'),
            '',
            [],
            [0, 1]
        );
        ...
        $rangeoptions['recurring']['helpbutton'] = ['recurring', 'customcertpwtelement_daterange'];
        $rangeoptions['datestring']['type'] = PARAM_NOTAGS;
        $rangeoptions['rangedelete']['type'] = PARAM_BOOL;

        $addstring = get_string('addrange', 'customcertpwtelement_daterange');
    ...
    public function validate_form_elements($data, $files) {
        ...
        $error = get_string('error:atleastone', 'customcertpwtelement_daterange');
        ...
        for ($i = 0; $i < $data['repeats']; $i++) {
            ...
            if (empty($data['datestring'][$i])) {
                ...
                $errors[$name] = get_string('error:datestring', 'customcertpwtelement_daterange');
            }

            // Check that end date is correctly set.
            if ( $data['startdate'][$i] >= $data['enddate'][$i] ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:enddate', 'customcertpwtelement_daterange');
            }
            ...
            if (!empty($data['recurring'][$i]) && $rangeperiod >= self::MAX_RECURRING_PERIOD ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:recurring', 'customcertpwtelement_daterange');
            }
        }
    ...
    public function render_html() {
        ...
        return element_helper::render_html_content($this, get_string('preview', 'customcertpwtelement_daterange', $this->get_name()));
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $data->dateitem)) {
            ...
            $DB->set_field('customcertpwt_elements', 'data', $this->save_unique_data($data), array('id' => $this->get_id()));
        }
    }
