...
/**
 * This file contains the customcertpwt element userpicture's core interaction API.
 *
 * @package    customcertpwtelement_userpicture
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_userpicture;

defined('MOODLE_INTERNAL') || die();

/**
 * The customcertpwt element userpicture's core interaction API.
 *
 * @package    customcertpwtelement_userpicture
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class element extends \mod_customcertpwt\element {
    ...
    public function render_form_elements($mform) {
        $mform->addElement('text', 'width', get_string('width', 'customcertpwtelement_userpicture'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'width', 'customcertpwtelement_userpicture');

        $mform->addElement('text', 'height', get_string('height', 'customcertpwtelement_userpicture'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'height', 'customcertpwtelement_userpicture');
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] < 0)) {
            $errors['width'] = get_string('invalidwidth', 'customcertpwtelement_userpicture');
        }

        // Check if height is not set, or not numeric or less than 0.
        if ((!isset($data['height'])) || (!is_numeric($data['height'])) || ($data['height'] < 0)) {
            $errors['height'] = get_string('invalidheight', 'customcertpwtelement_userpicture');
        }
