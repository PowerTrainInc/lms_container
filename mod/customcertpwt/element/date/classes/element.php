...
/**
 * This file contains the customcertpwt element date's core interaction API.
 *
 * @package    customcertpwtelement_date
 * @copyright  2021 PowerTrain Inc
 * @license    All Rights Reserved
 */

namespace customcertpwtelement_date;
 ...
 */
define('customcertpwt_DATE_COURSE_GRADE', '0');

/**
 * Date - Issue
 */
define('customcertpwt_DATE_ISSUE', '-1');

/**
 * Date - Completion
 */
define('customcertpwt_DATE_COMPLETION', '-2');

/**
 * Date - Course start
 */
define('customcertpwt_DATE_COURSE_START', '-3');

/**
 * Date - Course end
 */
define('customcertpwt_DATE_COURSE_END', '-4');

/**
 * Date - Current date
 */
define('customcertpwt_DATE_CURRENT_DATE', '-5');

/**
 * Date - Enrollment start
 */
define('customcertpwt_DATE_ENROLMENT_START', '-6');

/**
 * Date - Entrollment end
 */
define('customcertpwt_DATE_ENROLMENT_END', '-7');

require_once($CFG->dirroot . '/lib/grade/constants.php');

/**
 * The customcertpwt element date's core interaction API.
 *
 * @package    customcertpwtelement_date
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class element extends \mod_customcertpwt\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[customcertpwt_DATE_ISSUE] = get_string('issueddate', 'customcertpwtelement_date');
        $dateoptions[customcertpwt_DATE_CURRENT_DATE] = get_string('currentdate', 'customcertpwtelement_date');
        $completionenabled = $CFG->enablecompletion && ($COURSE->id == SITEID || $COURSE->enablecompletion);
        if ($completionenabled) {
            $dateoptions[customcertpwt_DATE_COMPLETION] = get_string('completiondate', 'customcertpwtelement_date');
        }
        $dateoptions[customcertpwt_DATE_ENROLMENT_START] = get_string('enrolmentstartdate', 'customcertpwtelement_date');
        $dateoptions[customcertpwt_DATE_ENROLMENT_END] = get_string('enrolmentenddate', 'customcertpwtelement_date');

        $dateoptions[customcertpwt_DATE_COURSE_START] = get_string('coursestartdate', 'customcertpwtelement_date');
        $dateoptions[customcertpwt_DATE_COURSE_END] = get_string('courseenddate', 'customcertpwtelement_date');
        $dateoptions[customcertpwt_DATE_COURSE_GRADE] = get_string('coursegradedate', 'customcertpwtelement_date');
        $dateoptions = $dateoptions + \mod_customcertpwt\element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'customcertpwtelement_date'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'customcertpwtelement_date');

        $mform->addElement('select', 'dateformat', get_string('dateformat', 'customcertpwtelement_date'), self::get_date_formats());
        $mform->addHelpButton('dateformat', 'dateformat', 'customcertpwtelement_date');
    ...
    public function render($pdf, $preview, $user) {
        ...
        $courseid = \mod_customcertpwt\element_helper::get_courseid($this->id);
        ...
        if ($preview) {
            ...
            if ($dateitem == customcertpwt_DATE_ISSUE) {
                $date = $issue->timecreated;
            } else if ($dateitem == customcertpwt_DATE_CURRENT_DATE) {
                $date = time();
            } else if ($dateitem == customcertpwt_DATE_COMPLETION) {
            ...
            } else if ($dateitem == customcertpwt_DATE_ENROLMENT_START) {
            ...
            } else if ($dateitem == customcertpwt_DATE_ENROLMENT_END) {
            ...
            } else if ($dateitem == customcertpwt_DATE_COURSE_START) {
                $date = $DB->get_field('course', 'startdate', array('id' => $courseid));
            } else if ($dateitem == customcertpwt_DATE_COURSE_END) {
                $date = $DB->get_field('course', 'enddate', array('id' => $courseid));
            } else {
                if ($dateitem == customcertpwt_DATE_COURSE_GRADE) {
                    $grade = \mod_customcertpwt\element_helper::get_course_grade_info(
                        $courseid,
                    ...
                    $grade = \mod_customcertpwt\element_helper::get_grade_item_info(
                        $gradeitemid,
                        $dateitem,
                        $user->id
                    );
                } else {
                    $grade = \mod_customcertpwt\element_helper::get_mod_grade_info(
                        $dateitem,
                        GRADE_DISPLAY_TYPE_DEFAULT,
                        $user->id
                    );
        ...
        if (!empty($date)) {
            \mod_customcertpwt\element_helper::render_content($pdf, $this, $this->get_date_format_string($date, $dateformat));
        }
    }
    ...
    public function render_html() {
        ...
        return \mod_customcertpwt\element_helper::render_html_content($this, $this->get_date_format_string(time(), $dateformat));
    }
     ...
     *
     * @param \restore_customcertpwt_activity_task $restore
     */
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $dateinfo->dateitem)) {
            ...
            $DB->set_field('customcertpwt_elements', 'data', $this->save_unique_data($dateinfo), array('id' => $this->get_id()));
        }
    }
    ...
    protected static function get_ordinal_number_suffix($day) {
        if (!in_array(($day % 100), array(11, 12, 13))) {
            switch ($day % 10) {
                // Handle 1st, 2nd, 3rd.
                case 1:
                    return get_string('numbersuffix_st_as_in_first', 'customcertpwtelement_date');
                case 2:
                    return get_string('numbersuffix_nd_as_in_second', 'customcertpwtelement_date');
                case 3:
                    return get_string('numbersuffix_rd_as_in_third', 'customcertpwtelement_date');
            }
        }
        return 'th';
    }
