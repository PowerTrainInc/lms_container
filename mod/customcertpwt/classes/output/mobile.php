...
namespace mod_customcertpwt\output;
...
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('customcertpwt', $cmid);
        $context = \context_module::instance($cm->id);
        self::require_capability($cm, $context, 'mod/customcertpwt:view');

        // Set some variables we are going to be using.
        $certificate = $DB->get_record('customcertpwt', ['id' => $cm->instance], '*', MUST_EXIST);
        ...
        list($certificate->intro, $certificate->introformat) = external_format_text($certificate->intro,
            $certificate->introformat, $context->id, 'mod_customcertpwt', 'intro');

        // Get any issues this person may have.
        $issue = false;
        if ($issues = $DB->get_records('customcertpwt_issues', ['userid' => $USER->id, 'customcertpwtid' => $certificate->id],
                'timecreated DESC')) {
            $issue = reset($issues);
        }
        ...
        $fileurl = "";
        if ($requiredtimemet) {
            $fileurl = new \moodle_url('/mod/customcertpwt/mobile/pluginfile.php', ['certificateid' => $certificate->id,
                'userid' => $USER->id]);
        ...
        $groups = [];
        $recipients = [];
        if (has_capability('mod/customcertpwt:viewreport', $context)) {
            $showreport = true;
            ...
            $recipients = \mod_customcertpwt\certificate::get_issues($certificate->id, $groupmode, $cm, 0, 0);
            foreach ($recipients as $recipient) {
                $recipient->displayname = fullname($recipient);
                $recipient->fileurl = new \moodle_url('/mod/customcertpwt/mobile/pluginfile.php', ['certificateid' => $certificate->id,
                    'userid' => $recipient->id]);
            }
        }
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_customcertpwt/mobile_view_activity_page', $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => ''
