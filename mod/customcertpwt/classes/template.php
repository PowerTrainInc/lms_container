...
/**
 * Class represents a customcertpwt template.
 *
...
class template {
    ...
    public function save($data) {
        ...
        $DB->update_record('customcertpwt_templates', $savedata);
    }
    ...
    public function add_page() {
        ...
        // Insert the page.
        return $DB->insert_record('customcertpwt_pages', $page);
    }
    ...
    public function save_page($data) {
        ...
        if ($pages = $DB->get_records('customcertpwt_pages', array('templateid' => $data->tid))) {
            ...
            foreach ($pages as $page) {
                ...
                $p->timemodified = $time;
                // Update the page.
                $DB->update_record('customcertpwt_pages', $p);
            }
        }
    }
        ...
        if ($elements = $DB->get_records_sql($sql, array('templateid' => $this->id))) {
            foreach ($elements as $element) {
                ...
                if ($e = \mod_customcertpwt\element_factory::get_element_instance($element)) {
                    ...
                    $DB->delete_records('customcertpwt_elements', array('id' => $element->id));
                }
            }
        }

        // Delete the pages.
        if (!$DB->delete_records('customcertpwt_pages', array('templateid' => $this->id))) {
            return false;
        }

        // Now, finally delete the actual template.
        if (!$DB->delete_records('customcertpwt_templates', array('id' => $this->id))) {
            return false;
        }
    ...
    public function delete_page($pageid) {
        ...
        // Get the page.
        $page = $DB->get_record('customcertpwt_pages', array('id' => $pageid), '*', MUST_EXIST);

        // Delete this page.
        $DB->delete_records('customcertpwt_pages', array('id' => $page->id));

        // The element may have some extra tasks it needs to complete to completely delete itself.
        if ($elements = $DB->get_records('customcertpwt_elements', array('pageid' => $page->id))) {
            foreach ($elements as $element) {
                // Get an instance of the element class.
                if ($e = \mod_customcertpwt\element_factory::get_element_instance($element)) {
                    $e->delete();
        ...
        $sql = "UPDATE {customcertpwt_pages}
    ...
    public function delete_element($elementid) {
        ...
        $element = $DB->get_record('customcertpwt_elements', array('id' => $elementid), '*', MUST_EXIST);
    ...
    public function generate_pdf(bool $preview = false, int $userid = null, bool $return = false) {
        ...
        if ($pages = $DB->get_records('customcertpwt_pages', array('templateid' => $this->id), 'sequence ASC')) {
            // Create the pdf object.
            $pdf = new \pdf();

            // Custom change.  Needed to change from get_record to get_records
			$customcertpwt = $DB->get_records('customcertpwt', ['templateid' => $this->id], '', '*', 0, 1);

            // If the template belongs to a certificate then we need to check what permissions we set for it.
            if (!empty($customcertpwt->protection)) {
                $protection = explode(', ', $customcertpwt->protection);
                $pdf->SetProtection($protection);
            }

            if (empty($customcertpwt->deliveryoption)) {
                $deliveryoption = certificate::DELIVERY_OPTION_INLINE;
            } else {
                $deliveryoption = $customcertpwt->deliveryoption;
            }
            ...
            if (empty($filename)) {
                $filename = get_string('certificate', 'customcertpwt');
            }
    ...
    public function copy_to_template($copytotemplateid) {
        ...
        // Get the pages for the template, there should always be at least one page for each template.
        if ($templatepages = $DB->get_records('customcertpwt_pages', array('templateid' => $this->id))) {
            ...
            foreach ($templatepages as $templatepage) {
                ...
                $page->id = $DB->insert_record('customcertpwt_pages', $page);
                // Now go through the elements we want to load.
                if ($templateelements = $DB->get_records('customcertpwt_elements', array('pageid' => $templatepage->id))) {
                    foreach ($templateelements as $templateelement) {
                        ...
                        $element->id = $DB->insert_record('customcertpwt_elements', $element);
    ...
    public function get_cm() {
        ...
        if ($context->contextlevel === CONTEXT_MODULE) {
            return get_coursemodule_from_id('customcertpwt', $context->instanceid, 0, false, MUST_EXIST);
        }
    ...
    public function require_manage() {
        require_capability('mod/customcertpwt:manage', $this->get_context());
    }
     ...
     * @return \mod_customcertpwt\template the template object
     */
    public static function create($templatename, $contextid) {
        ...
        $template->id = $DB->insert_record('customcertpwt_templates', $template);

        return new \mod_customcertpwt\template($template);
    }
}
