...
/**
 * Provides functionality needed by customcertpwt activities.
 *
...
class certificate {
     ...
     *      If you want to display all customcertpwts on a page set this to 0.
     */
    const customcertpwt_PER_PAGE = '50';

    /**
     * Handles setting the protection field for the customcertpwt
     *
     * @param \stdClass $data
    ...
    /**
     * Handles uploading an image for the customcertpwt module.
     *
    ...
    public static function upload_files($draftitemid, $contextid, $filearea = 'image') {
        ...
        file_save_draft_area_files($draftitemid, $contextid, 'mod_customcertpwt', $filearea, 0);
    }
    ...
    /**
     * Returns a list of issued customcertpwts.
     *
     ...
     */
    public static function get_issues($customcertpwtid, $groupmode, $cm, $limitfrom, $limitnum, $sort = '') {
        global $DB;
        ...
        // Add the conditional SQL and the customcertpwtid to form all used parameters.
        $allparams = $conditionsparams + array('customcertpwtid' => $customcertpwtid);
        ...
        $sql = "SELECT $ufields, ci.id as issueid, ci.code, ci.timecreated
                  FROM {user} u
            INNER JOIN {customcertpwt_issues} ci
                    ON u.id = ci.userid
                 WHERE u.deleted = 0
                   AND ci.customcertpwtid = :customcertpwtid
                       $conditionssql";
        if ($sort) {
            $sql .= "ORDER BY " . $sort;
        } else {
    ...
    /**
     * Returns the total number of issues for a given customcertpwt.
     *
     * @param int $customcertpwtid
     * @param \stdClass $cm the course module
     ...
     */
    public static function get_number_of_issues($customcertpwtid, $cm, $groupmode) {
        global $DB;
    ...
    public static function get_conditional_issues_sql($cm, $groupmode) {
        ...
        // Get all users that can manage this customcertpwt to exclude them from the report.
        $context = \context_module::instance($cm->id);
        $conditionssql = '';
        $conditionsparams = array();

        // Get all users that can manage this certificate to exclude them from the report.
        $certmanagers = array_keys(get_users_by_capability($context, 'mod/customcertpwt:manage', 'u.id'));
    ...
    public static function issue_certificate($certificateid, $userid) {
        ...
        $issue = new \stdClass();
        $issue->userid = $userid;
        $issue->customcertpwtid = $certificateid;
        $issue->code = self::generate_code();
        $issue->emailed = 0;
        $issue->timecreated = time();

        // Insert the record into the database.
        return $DB->insert_record('customcertpwt_issues', $issue);
    }
    ...
    public static function generate_code() {
        ...
        while (!$uniquecodefound) {
            if (!$DB->record_exists('customcertpwt_issues', array('code' => $code))) {
