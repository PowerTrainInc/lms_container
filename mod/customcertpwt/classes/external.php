...
class external extends \external_api {
    ...
    public static function save_element($templateid, $elementid, $values) {
        ...
        $template = $DB->get_record('customcertpwt_templates', array('id' => $templateid), '*', MUST_EXIST);
    ...
    /**
     * Handles deleting a customcertpwt issue.
     *
    ...
    public static function delete_issue($certificateid, $issueid) {
        ...
        $issue = $DB->get_record('customcertpwt_issues', ['id' => $issueid, 'customcertpwtid' => $certificateid], '*', MUST_EXIST);

        $cm = get_coursemodule_from_instance('customcertpwt', $certificate->id, 0, false, MUST_EXIST);
        ...
        // Delete the issue.
        return $DB->delete_records('customcertpwt_issues', ['id' => $issue->id]);
    }
