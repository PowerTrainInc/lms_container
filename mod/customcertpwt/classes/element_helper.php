...
class element_helper {
     ...
     */
    const customcertpwt_REF_POINT_TOPLEFT = 0;

    /**
     * @var int the top-center of element
     */
    const customcertpwt_REF_POINT_TOPCENTER = 1;

    /**
     * @var int the top-left of element
     */
    const customcertpwt_REF_POINT_TOPRIGHT = 2;

    /**
     * Common behaviour for rendering specified content on the pdf.
     *
     * @param \pdf $pdf the pdf object
     * @param \mod_customcertpwt\element $element the customcertpwt element
    ...
    public static function render_content($pdf, $element, $content) {
        ...
        switch ($refpoint) {
            case self::customcertpwt_REF_POINT_TOPRIGHT:
                ...
                break;
            case self::customcertpwt_REF_POINT_TOPCENTER:
    ...
    public static function render_form_element_font($mform) {
        $mform->addElement('select', 'font', get_string('font', 'customcertpwt'), \mod_customcertpwt\certificate::get_fonts());
        $mform->setType('font', PARAM_TEXT);
        $mform->setDefault('font', 'times');
        $mform->addHelpButton('font', 'font', 'customcertpwt');
        $mform->addElement('select', 'fontsize', get_string('fontsize', 'customcertpwt'),
            \mod_customcertpwt\certificate::get_font_sizes());
        $mform->setType('fontsize', PARAM_INT);
        $mform->setDefault('fontsize', 12);
        $mform->addHelpButton('fontsize', 'fontsize', 'customcertpwt');
    }
    ...
    public static function render_form_element_colour($mform) {
        $mform->addElement('customcertpwt_colourpicker', 'colour', get_string('fontcolour', 'customcertpwt'));
        $mform->setType('colour', PARAM_RAW); // Need to validate that this is a valid colour.
        $mform->setDefault('colour', '#000000');
        $mform->addHelpButton('colour', 'fontcolour', 'customcertpwt');
    }
    ...
    public static function render_form_element_position($mform) {
        $mform->addElement('text', 'posx', get_string('posx', 'customcertpwt'), array('size' => 10));
        $mform->setType('posx', PARAM_INT);
        $mform->setDefault('posx', 0);
        $mform->addHelpButton('posx', 'posx', 'customcertpwt');
        $mform->addElement('text', 'posy', get_string('posy', 'customcertpwt'), array('size' => 10));
        $mform->setType('posy', PARAM_INT);
        $mform->setDefault('posy', 0);
        $mform->addHelpButton('posy', 'posy', 'customcertpwt');
    }
    ...
    public static function render_form_element_width($mform) {
        $mform->addElement('text', 'width', get_string('elementwidth', 'customcertpwt'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'elementwidth', 'customcertpwt');
    }
    ...
    public static function render_form_element_refpoint($mform) {
        ...
        $refpointoptions[self::customcertpwt_REF_POINT_TOPLEFT] = get_string('topleft', 'customcertpwt');
        $refpointoptions[self::customcertpwt_REF_POINT_TOPCENTER] = get_string('topcenter', 'customcertpwt');
        $refpointoptions[self::customcertpwt_REF_POINT_TOPRIGHT] = get_string('topright', 'customcertpwt');

        $mform->addElement('select', 'refpoint', get_string('refpoint', 'customcertpwt'), $refpointoptions);
        $mform->setType('refpoint', PARAM_INT);
        $mform->setDefault('refpoint', self::customcertpwt_REF_POINT_TOPCENTER);
        $mform->addHelpButton('refpoint', 'refpoint', 'customcertpwt');
    }
    ...
    public static function validate_form_element_colour($data) {
        ...
        if (!self::validate_colour($data['colour'])) {
            $errors['colour'] = get_string('invalidcolour', 'customcertpwt');
        }
        return $errors;
    }
    ...
    public static function validate_form_element_position($data) {
        ...
        if ((!isset($data['posx'])) || (!is_numeric($data['posx'])) || ($data['posx'] < 0)) {
            $errors['posx'] = get_string('invalidposition', 'customcertpwt', 'X');
        }
        // Check if posy is not set, or not numeric or less than 0.
        if ((!isset($data['posy'])) || (!is_numeric($data['posy'])) || ($data['posy'] < 0)) {
            $errors['posy'] = get_string('invalidposition', 'customcertpwt', 'Y');
        }
    ...
    public static function validate_form_element_width($data) {
        ...
        if (isset($data['width']) && $data['width'] < 0) {
            $errors['width'] = get_string('invalidelementwidth', 'customcertpwt');
        }
    ...
    /**
     * Helper function that returns the sequence on a specified customcertpwt page for a
    ...
    public static function get_courseid($elementid) {
        ...
        $sql = "
            ...
            INNER JOIN {customcertpwt_pages} cp
                ON c.templateid = cp.templateid
            INNER JOIN {customcertpwt_elements} ce
                ON cp.id = ce.pageid
    ...
    public static function get_context(int $elementid) : \context {
        ...
        $sql = "SELECT ct.contextid
                  FROM {customcertpwt_templates} ct
            INNER JOIN {customcertpwt_pages} cp
    ...
    public static function get_available_element_types() {
        ...
        $elementdir = "$CFG->dirroot/mod/customcertpwt/element";
        ...
        if ($gradeitems = \grade_item::fetch_all(['courseid' => $course->id])) {
            foreach ($gradeitems as $gi) {
                ...
                if ($gi->is_external_item() && !$gi->is_outcome_item()) {
                    ...
                    $arrgradeitems[$cm->id] = get_string('activity', 'mod_customcertpwt') . ' : ' . $gi->get_name();
                } else if ($gi->is_external_item() && $gi->is_outcome_item()) {
                    // Get the name of the activity.
                    $optionname = get_string('gradeoutcome', 'mod_customcertpwt') . ' : '  . $modname . " - " . $gi->get_name();
