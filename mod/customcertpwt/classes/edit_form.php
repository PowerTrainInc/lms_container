...
/**
 * This file contains the form for handling the layout of the customcertpwt instance.
 *
...
/**
 * The form for handling the layout of the customcertpwt instance.
 *
 * @package    mod_customcertpwt
 * @copyright  2021 PowerTrain Inc
 * @license    All rights Reserved
...
class edit_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('name', 'customcertpwt'), 'maxlength="255"');
        ...
        if (isset($this->_customdata['tid'])) {
            ...
            if ($pages = $DB->get_records('customcertpwt_pages', array('templateid' => $this->tid), 'sequence')) {
                $this->numpages = count($pages);
                foreach ($pages as $p) {
                    $this->add_customcertpwt_page_elements($p);
                }
            }
            ...
            $page->id = 0;
            $page->sequence = 1;
            $this->add_customcertpwt_page_elements($page);
        }

        // Link to add another page, only display it when the template has been created.
        if (isset($this->_customdata['tid'])) {
            $addpagelink = new \moodle_url('/mod/customcertpwt/edit.php',
                array(
                    'tid' => $this->tid,
                    'aid' => 1,
                    'action' => 'addpage',
            ...
            $icon = $OUTPUT->pix_icon('t/switch_plus', get_string('addcertpage', 'customcertpwt'));
            $addpagehtml = \html_writer::link($addpagelink, $icon . get_string('addcertpage', 'customcertpwt'));
        ...
        $group[] = $mform->createElement('submit', 'previewbtn', get_string('savechangespreview', 'customcertpwt'), array(), false);
    ...
    public function definition_after_data() {
        ...
        // Check that we are updating a current customcertpwt.
        if ($this->tid) {
            // Get the pages for this customcertpwt.
            if ($pages = $DB->get_records('customcertpwt_pages', array('templateid' => $this->tid))) {
        ...
        foreach ($data as $key => $value) {
            if (strpos($key, 'pagewidth_') !== false) {
                ...
                if ((!isset($data[$widthid])) || (!is_numeric($data[$widthid])) || ($data[$widthid] <= 0)) {
                    $errors[$widthid] = get_string('invalidwidth', 'customcertpwt');
                }
            }
            if (strpos($key, 'pageheight_') !== false) {
                ...
                if ((!isset($data[$heightid])) || (!is_numeric($data[$heightid])) || ($data[$heightid] <= 0)) {
                    $errors[$heightid] = get_string('invalidheight', 'customcertpwt');
                }
            }
            if (strpos($key, 'pageleftmargin_') !== false) {
                // Validate that the left margin is a valid value.
                if (isset($data[$key]) && ($data[$key] < 0)) {
                    $errors[$key] = get_string('invalidmargin', 'customcertpwt');
                }
            }
     ...
     *
     * @param \stdClass $page the customcertpwt page
     */
    protected function add_customcertpwt_page_elements($page) {
        global $DB, $OUTPUT;
        ...
        if ($this->numpages > 1) {
            $mform->addElement('header', 'page_' . $page->id, get_string('page', 'customcertpwt', $page->sequence));
        }

        $editlink = '/mod/customcertpwt/edit.php';
        $editlinkparams = array('tid' => $this->tid, 'sesskey' => sesskey());
        $editelementlink = '/mod/customcertpwt/edit_element.php';
        ...
        $mform->addElement('text', 'pagewidth_' . $page->id, get_string('width', 'customcertpwt'));
        ...
        $mform->addHelpButton('pagewidth_' . $page->id, 'width', 'customcertpwt');

        $mform->addElement('text', 'pageheight_' . $page->id, get_string('height', 'customcertpwt'));
        ...
        $mform->addHelpButton('pageheight_' . $page->id, 'height', 'customcertpwt');

        $mform->addElement('text', 'pageleftmargin_' . $page->id, get_string('leftmargin', 'customcertpwt'));
        $mform->setType('pageleftmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pageleftmargin_' . $page->id, 0);
        $mform->addHelpButton('pageleftmargin_' . $page->id, 'leftmargin', 'customcertpwt');

        $mform->addElement('text', 'pagerightmargin_' . $page->id, get_string('rightmargin', 'customcertpwt'));
        $mform->setType('pagerightmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pagerightmargin_' . $page->id, 0);
        $mform->addHelpButton('pagerightmargin_' . $page->id, 'rightmargin', 'customcertpwt');

        // Check if there are elements to add.
        if ($elements = $DB->get_records('customcertpwt_elements', array('pageid' => $page->id), 'sequence ASC')) {
            // Get the total number of elements.
            $numelements = count($elements);
            // Create a table to display these elements.
            $table = new \html_table();
            $table->attributes = array('class' => 'generaltable elementstable');
            $table->head  = array(get_string('name', 'customcertpwt'), get_string('type', 'customcertpwt'), get_string('actions'));
            $table->align = array('left', 'left', 'left');
            // Loop through and add the elements to the table.
            foreach ($elements as $element) {
                $elementname = new \core\output\inplace_editable('mod_customcertpwt', 'elementname', $element->id,
            ...
            $link = \html_writer::link(new \moodle_url('/mod/customcertpwt/rearrange.php', array('pid' => $page->id)),
                get_string('rearrangeelements', 'customcertpwt'));
            // Add the table to the form.
            $mform->addElement('static', 'elements_' . $page->id, get_string('elements', 'customcertpwt'), \html_writer::table($table)
                . \html_writer::tag( 'div', $link));
            $mform->addHelpButton('elements_' . $page->id, 'elements', 'customcertpwt');
        }

        $group = array();
        $group[] = $mform->createElement('select', 'element_' . $page->id, '', element_helper::get_available_element_types());
        $group[] = $mform->createElement('submit', 'addelement_' . $page->id, get_string('addelement', 'customcertpwt'),
            array(), false);
        ...
        if ($this->numpages > 1) {
            ...
            $icon = $OUTPUT->pix_icon('t/delete', get_string('deletecertpage', 'customcertpwt'));
            $deletepagehtml = \html_writer::link($deletelink, $icon . get_string('deletecertpage', 'customcertpwt'));
