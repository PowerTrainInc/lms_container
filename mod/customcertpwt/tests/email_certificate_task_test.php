 ...
 * @license    All Rights Reserved
 */
class mod_customcertpwt_task_email_certificate_task_testcase extends advanced_testcase {
    ...
    public function test_email_certificates_no_elements() {
        ...
        $this->getDataGenerator()->create_module('customcertpwt', ['course' => $course->id, 'emailstudents' => 1]);
        ...
        $task = new \mod_customcertpwt\task\email_certificate_task();
        $task->execute();
    ...
    public function test_email_certificates_no_cap() {
        ...
        unassign_capability('mod/customcertpwt:receiveissue', $roleids['student']);

        // Create a custom certificate.
        $customcertpwt = $this->getDataGenerator()->create_module('customcertpwt', ['course' => $course->id, 'emailstudents' => 1]);
        ...
        $element->name = 'Image';
        $DB->insert_record('customcertpwt_elements', $element);
    ...
    public function test_email_certificates_students() {
        ...
        \mod_customcertpwt\certificate::issue_certificate($customcertpwt->id, $user1->id);

        // Confirm there is only one entry in this table.
        $this->assertEquals(1, $DB->count_records('customcertpwt_issues'));
    ...
    public function test_email_certificates_others() {
        ...
        // Create a custom certificate.
        $customcertpwt = $this->getDataGenerator()->create_module('customcertpwt', array('course' => $course->id,
            'emailothers' => 'testcustomcertpwt@example.com, doo@dah'));
        ...
        $this->assertEquals($CFG->noreplyaddress, $emails[0]->from);
        $this->assertEquals('testcustomcertpwt@example.com', $emails[0]->to);

        $this->assertEquals($CFG->noreplyaddress, $emails[1]->from);
        $this->assertEquals('testcustomcertpwt@example.com', $emails[1]->to);
    }
    ...
    public function test_email_certificates_students_not_visible() {
        ...
        assign_capability('mod/customcertpwt:view', CAP_PROHIBIT, $roleids['student'], \context_course::instance($course->id));
    ...
    public function test_email_certificates_students_havent_met_required_time() {
        ...
        // Create a custom certificate.
        $customcertpwt = $this->getDataGenerator()->create_module('customcertpwt', array('course' => $course->id, 'emailstudents' => 1,
            'requiredtime' => '60'));
