...
function ddmarker(array $config, \stdClass $event, \stdClass $questionattempt, \stdClass $question) {
    global $DB;
    ...
    $subquestions = $DB->get_records('qtype_ddmarker', array('questionid' => $question->id), 'id ASC');

    for ($i = 0; $i < count($parsedRightAnswer); $i++) {
        $id = $i + 1;
        $sourceID = $id < 10 ? 'source0'.$id : 'source'.$id;
        $targetID = $id < 10 ? 'target0'.$id : 'target'.$id;
        $str = explode(' -> ', $parsedRightAnswer[$i]);
        $src = explode('. ', $str[0]);
        $source[] = [
            'id' => $sourceID,
            'description' => [
